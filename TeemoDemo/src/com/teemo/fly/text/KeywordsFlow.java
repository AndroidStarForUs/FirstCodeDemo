...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
    private final String TAg = KeywordsFlow.class.getSimpleName();
    ...
    public static final int MAX = /*10*/1;
    ...
    public AnimationSet getAnimationSet(int[] xy, int xCenter, int yCenter, int type) {
        // 创建Animation的Set集合，True:允许Set中的Animation使用关联的插入器
        Log.d(TAg, "xCenter = " + xCenter + " yCenter = " + yCenter);
        AnimationSet animSet = new AnimationSet(true);
        animSet.setInterpolator(interpolator);
        if (type == OUTSIDE_TO_LOCATION) {
            Log.e(TAg, "OUTSIDE_TO_LOCATION");
            animSet.addAnimation(animAlpha2Opaque);
            animSet.addAnimation(animScaleLarge2Normal);
            Log.d(TAg, "fromXDelta = " + ((xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1));
            Log.d(TAg, "toXDelta = " + 0);
            Log.d(TAg, "fromYDelta = " + ((xy[IDX_Y] - yCenter) << 1));
            Log.d(TAg, "toYDelta = " + 0);
            ...
            Log.e(TAg, "LOCATION_TO_OUTSIDE");
            animSet.addAnimation(animAlpha2Transparent);
            animSet.addAnimation(animScaleNormal2Large);
            Log.d(TAg, "fromXDelta = " + 0);
            Log.d(TAg, "toXDelta = " + ((xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1));
            Log.d(TAg, "fromYDelta = " + 0);
            Log.d(TAg, "toYDelta = " + ((xy[IDX_Y] - yCenter) << 1));
